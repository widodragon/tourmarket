...
const ListingStayDetailPage: FC<ListingStayDetailPageProps> = ({
  ...
  const getDetailTcsProduct = () => {
    let cid = decode(location.search.split('key=')[1]);
    POST('/mtcs/index', { cid: cid })
      .then((tcs) => {
        if (Object.keys(tcs.result).length > 0) {
          setTcsProduct(tcs.result);
          let getMap = getSrc(tcs.result.tcsConfig?.embedMap);
          setMap(getMap);
        }
      })
      .catch((error) => {
      ...
      <div className="listingSectionSidebar__wrap shadow-xl">
        ...
        {/* <div className="flex justify-between">
          <span className="text-3xl font-semibold">
            Rp {convertNumbThousand(tcsProduct.minPrice) + " - " + convertNumbThousand(tcsProduct.maxPrice)}
          </span>
          <StartRating point={tcsProduct.rating} reviewCount={tcsProduct.countReview} />
        </div> */}
  ...
  return (
    ...
    >
      ...
      {!isPreviewMode && <MobileFooterSticky tcsProducts={tcsProduct} />}
