...
const ReservationPage: FC<ReservationPageProps> = ({ className = '' }) => {
  ...
  const getProductTcsInformation = async () => {
    ...
    if (key) {
      ...
      POST('/mproduct/list/datev2', body)
        .then((tcs) => {
  ...
  if (loading) {
    ...
    return (
      <div
        className={`nc-ReservationPage ${className}`}
        data-nc-id="ReservationPage"
      >
        <Helmet>
