...
export interface EventTargetPageProps {
  className?: string;
  isPreviewMode?: boolean;
}

const EventTargetPage: FC<EventTargetPageProps> = ({
  className = '',
  isPreviewMode,
}) => {
  ...
  const code = router.location.pathname;
  const windowSize = useWindowSize();

  React.useEffect(() => {
    if (Object.keys(tcsProduct).length === 0 && code) {
      getDetailTcsProductEvent();
    }
  }, [tcsProduct, code]);
  ...
  const getDetailTcsProductEvent = () => {
    let config = getStorage(CONFIG_STR);
    let body = {
      eventCode: code?.split("/")[2],
    ...
    POST('/mevent-tcsdetail/list/index', body)
      .then((tcs) => {
        if (Object.keys(tcs.result).length > 0) {
          ...
          setGalleryImgs(tcs?.result?.imageGallery);
          setEventProduct(tcs?.result?.[0]);
          let getMap = getSrc(tcs?.result?.embedMap);
          setMap(getMap);
        }
      })
      .catch((error) => {
  ...
  return (
    <div
      className={`ListingDetailPage nc-EventTargetPage ${className}`}
      data-nc-id="EventTargetPage"
    >
      {/* SINGLE HEADER */}
      <Helmet>
      ...
      <>
        ...
        <ModalPhotos
          ...
          uniqueClassName="nc-EventTargetPage-modalPhotos"
        />
      </>
